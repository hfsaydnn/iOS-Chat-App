//
//  CBZTextFieldAlertViewController.swift
//  InstagramClone
//
//  Created by Mayil Kannan on 29/05/21.
//

import SwiftUI
import Combine

class CBZTextFieldAlertViewController: UIViewController {
    
    init(title: String, message: String?, text: Binding<String?>, isOkayPressed: Binding<Bool>, isPresented: Binding<Bool>?) {
        self.alertTitle = title
        self.message = message
        self._text = text
        self._isOkayPressed = isOkayPressed
...
extension TextFieldAlert: UIViewControllerRepresentable {
    ...
    typealias UIViewControllerType = CBZTextFieldAlertViewController
    
    func makeUIViewController(context: UIViewControllerRepresentableContext<TextFieldAlert>) -> UIViewControllerType {
        CBZTextFieldAlertViewController(title: title, message: message, text: $text, isOkayPressed: $isOkayPressed, isPresented: isPresented)
    }
