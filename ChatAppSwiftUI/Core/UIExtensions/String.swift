...
struct kMentionsConfig {
    static let mentionsColorCode = "#1A499F"
    static let mentionsNSColorCode = "0.101961 0.286275 0.623529 1"
}
...
extension String {
    ...
    func htmlToAttributedString(textColor: UIColor) -> NSAttributedString {
        ...
        do {
            ...
            var modifiedFont = self.replacingOccurrences(of: "<font color='\(kMentionsConfig.mentionsColorCode)'>", with: "InstamobileHtmlStringTag1")
            ...
            modifiedFont = modifiedFont.replacingOccurrences(of: "InstamobileHtmlStringTag1", with: "<font color='\(kMentionsConfig.mentionsColorCode)'>")
...
extension NSAttributedString {
    func fetchAttributedText(allTagUsers: [String]) -> String {
        ...
        for (index, str) in self.string.enumerated() {
            ...
            if checkBlue.contains(kMentionsConfig.mentionsNSColorCode) {
                ...
                if index == self.string.count - 1 {
                    if !isPreviuosAttrbiutedString.isEmpty {
                        ...
                        if let filterTagUser = filterTagUser {
                            ...
                            allSubstrings += "<font color='\(kMentionsConfig.mentionsColorCode)'>\(filterTagUser)</font>"
