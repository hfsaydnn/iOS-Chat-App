...
protocol ATCChatServiceConfigProtocol {
    ...
    var isVideoCallingEnabled: Bool {get}
    var isAudioCallingEnabled: Bool {get}
    func isAudioVideoCallEnabled() -> Bool
...
let kDismissChatNotificationName = NSNotification.Name(rawValue: "kDismissChatNotificationName")
...
class ATCChatThreadViewController: MessagesViewController, MessagesDataSource, ATCChatAudioRecordingProtocol, AVAudioRecorderDelegate {
    ...
    let audioVideoChatPresenter: ATCAudioVideoChatPresenterProtocol?
    ...
    init(user: ATCUser,
         ...
         recipients: [ATCUser] = [],
         audioVideoChatPresenter: ATCAudioVideoChatPresenterProtocol?) {
        self.user = user
        ...
        self.audioVideoChatPresenter = audioVideoChatPresenter
    ...
    override func viewDidLoad() {
        ...
        if chatServiceConfig?.isAudioMessagesEnabled ?? false {
            ...
            replyItem = ReplyItemView(frame: CGRect(0, 0, screen.width, 60))
            replyItem.delegate = self
            messageInputBar.setStackViewItems([recordItem], forStack: .bottom, animated: true)
            messageInputBar.setStackViewItems([replyItem], forStack: .top, animated: true)
            recordItem?.isHidden = true
            replyItem?.isHidden = true
        }
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(dismissChatVC), name: kDismissChatNotificationName, object: nil)
    }
    ...
    fileprivate func saveSeenStatusIfNeeded(message: ATChatMessage) {
        if (message.lastMessageSeeners.filter { $0.uid == self.user.uid }).isEmpty {
            self.reference?.document(message.messageId).getDocument(completion: { (snapshot, error) in
                ...
                channelRef.setData(["lastMessageSeeners": lastMessageSeeners], merge: true)
            })
        }
    }
...
//            }
    ...
    private func videoCallBarButtonItem() -> UIBarButtonItem {
        let videoCallBtn = UIButton(type: .custom)
        videoCallBtn.setImage(UIImage.localImage("video-call-icon", template: true), for: .normal)
        videoCallBtn.addTarget(self, action: #selector(videoCallButtonTapped), for: .touchUpInside)
        let item = UIBarButtonItem(customView: videoCallBtn)
        item.customView?.translatesAutoresizingMaskIntoConstraints = false
        item.customView?.snp.makeConstraints { (maker) in
            maker.width.height.equalTo(30)
        }
        return item
    }
    
    private func audioCallBarButtonItem() -> UIBarButtonItem {
        let audioCallBtn = UIButton(type: .custom)
        audioCallBtn.setImage(UIImage.localImage("audio-call-icon", template: true), for: .normal)
        audioCallBtn.addTarget(self, action: #selector(audioCallButtonTapped), for: .touchUpInside)
        let item = UIBarButtonItem(customView: audioCallBtn)
        item.customView?.translatesAutoresizingMaskIntoConstraints = false
        item.customView?.snp.makeConstraints { (maker) in
            maker.width.height.equalTo(30)
        }
    ...
    private func updateNavigationBar() {
        if self.channel.participants.count > 2 || !self.channel.groupCreatorID.isEmpty {
            ...
            if chatServiceConfig?.isVideoCallingEnabled ?? false {
                rightBarButtonItems.append(videoCallBarButtonItem())
            }
            if chatServiceConfig?.isAudioCallingEnabled ?? false {
                rightBarButtonItems.append(audioCallBarButtonItem())
            }
    ...
    @objc private func videoCallButtonTapped() {
        audioVideoChatPresenter?.startCall(in: self, chatType: .video, channel: self.channel, user: user, otherUser: otherUser())
    }
    
    @objc private func audioCallButtonTapped() {
        audioVideoChatPresenter?.startCall(in: self, chatType: .audio, channel: self.channel, user: user, otherUser: otherUser())
    }
...
extension ATCChatThreadViewController: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        let message = ATChatMessage(messageId: UUID().uuidString,
                                    ...
                                    inReplyToMessage: self.inReplyToMessage)
        save(message)
...
extension ATCChatThreadViewController: MessagesViewControllerDelegate, ReplyItemDelegate {
    func didTapOnCloseButton() {
        replyItem.isHidden = true
        replyItem.contentLabel.text = ""
    }
    
    func handleSelectedMessage(with cell: TextMessageCell, message: MessageType?) {
        let recipient = recipients.first{ $0.uid == message?.sender.senderId }
        replyItem.isHidden = false
        let replyRecipients = recipient?.firstName ?? ""
        self.inReplyToMessage = cell.messageLabel.text
        replyItem.configure(with: replyRecipients, content: cell.messageLabel.text)
    }
}
