...
class ATCAddress: ATCGenericBaseModel {
    ...
    required init(shopifyDict: [String: Any]) {
        self.firstName = (shopifyDict["firstName"] as? String) ?? ""
        self.lastName = (shopifyDict["lastName"] as? String) ?? ""
        if let firstName = firstName, let lastName = lastName {
            self.name = firstName.isEmpty ? lastName : (firstName + " " + lastName)
        }
        self.line1 = (shopifyDict["address"] as? String) ?? ""
        self.line2 = (shopifyDict["apartment"] as? String) ?? ""
        self.city = (shopifyDict["city"] as? String) ?? ""
        self.country = (shopifyDict["country"] as? String) ?? ""
        self.state = (shopifyDict["state"] as? String) ?? ""
        self.postalCode = (shopifyDict["zipcode"] as? String) ?? ""
        self.phone = (shopifyDict["phone"] as? String) ?? ""
        self.addressID = shopifyDict["addressID"] as? String
    }
