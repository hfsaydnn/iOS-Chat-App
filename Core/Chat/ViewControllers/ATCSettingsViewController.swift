...
class ATCSettingsViewController: QuickTableViewController {
    ...
    private var pushNotificationManager: ATCPushNotificationManager?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        pushNotificationManager = ATCPushNotificationManager(user: user)
    ...
    func handleSaveButton() {
        ...
        usersRef.setData(userSettingsJSON) { [weak self] (error) in
            guard let self = self else { return }
            hud.dismiss()
            if error == nil {
                if self.userSettingsJSON[self.push_notification_key] ?? false {
